...
When a source fails to load or the data does not match the quiz template, the app shows an error that includes a developer hint: columns A–F (numbering, title, question, instruction, question type, correct choice), then from column G onward headers that begin with `Choice` are choices and headers that contain `Hint` are hints (see `gsheets/Test/sample-quiz.csv`).
...
The sheet must have the first row reserved for column headings. Columns A–F are fixed by position; from column G onward the **header text** decides whether a column is a choice or a hint. This applies to every source (Google Sheet, local CSV, user-pasted CSV):
...
- Correct Choice. This is the index number 1,2,3,4,etc that corresponds to the **Choice** columns (counting only columns whose header begins with the word `Choice`, in left-to-right order) and that is the actually correct answer. You can have multiple correct choices with comma separated values in the choice of Select all that apply (SATA):
Eg. 2
Eg. 2,3,4
...
- From column G onward: any header that **begins with the word `Choice`** (for example `Choice 1`, `Choice 2`) is a choice column. You can have four such columns for a standard a,b,c,d multiple choice, or two for True/False (put `True` / `False` in the cells; keep the headers as `Choice 1` / `Choice 2`). For flash cards you can use two Choice columns with "Got it correct" / "Got it wrong" and set the Type column to `Flash Card` (end of quiz will review your wrong questions).
- From column G onward: any header that **contains the word `Hint`** (for example `Hint`, `Hint 1`) is a hint column. Hint text is shown on the question as an initially collapsed expandable section. If there are multiple Hint columns, their non-empty cells are appended together.
...
Just have the choices entered under headers that begin with `Choice`. You can have more than 4 multiple choices or less. The app will figure out how many choices to render based on how many Choice columns you fill in. Optional `Hint` columns (header contains `Hint`) show as a collapsed expandable hint on the question.
...
When the user clicks a quiz at index.php, then the spreadsheet tab's cell values will be loaded into javascript as a JSON intermediate form. Over at Javascript, the app expects columns A–F in a fixed order. From column G onward, the header row decides the role of each column: headers that begin with the word `Choice` are answer choices; headers that contain the word `Hint` are optional hints (rendered as a collapsed expandable block on the question; multiple hint columns are appended). The same header rules apply for Google Sheet, local CSV, and user-pasted CSV sources.

The cell in the "Question Type" column could have "Picture" then the question text will be rendered as a picture rather than a text. The column "Correct Choice" should have a number indicating the correct Choice column counting from 1 to N among Choice-named columns only. You are not limited to a standard multiple choices question with 4 choices. If you have 2 Choice columns, you can make the question appear as a True or False by putting "True" and "False" in those cells. You're not forced to have the same number of choices at every question (every row). Regardsless, the "Correct Choice" column is a number that points to the correct Choice column. In this case for True or False, it would be either 1 or 2. With the correct choice number, the app will listen for a click event among the choices in the DOM, and then it will compare that clicked DOM's index to the correct choice index from the spreadsheet. In a similiar note, if your Question Type is "Flash card", the question will be rendered as a DOM that you can click to flip between front and back sides, and your choices can be "I got it right" and "I got it wrong" so you can track the score and also review what you've got wrong.
...
```
[
    ["Optional Sorting Rank (-1 for hide from quiz)", "Title/Subcategory", "Question", "Instruction", "Question Type", "Correct Choice", "Choice 1", "Choice 2", "Choice 3", "Choice 4", "Choice 5", "Choice 6", "Choice 7", "Hint"],
    ["", "Concepts", "Which distribution point involves businesses selling products directly to customers without involving intermediaries?", "Select the option that corresponds to the correct answer.", "Multiple Choice", "3", "Retail Stores", "Warehouses", "Direct Sales", "Wholesalers", "", "", "", "No middlemen between seller and customer"],
