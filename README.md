...
When the user clicks a quiz at index.php, then all the spreadsheet tabs cell values will be dumped into a json form for javascript to consume. Over at javascript, it expects columns to have certain meanings associated with them based on the position of the column. In particularly, a column could for question type could be named Picture then the question text will be rendered differently, in particular with an img tag.  The column Correct choice will let the app listen for click even and compare if the index of the clicked answer matches the correct choice index from the spreadsheet (count starts at 1). In addition, that correct choice column can have comma separated indexes instead of a single index, then the question becomes select all that apply and a button renders that let you finish selecting multiple SATA choices. It can also with some common sense tweaking you can make it into a True/False question or Flash Card question. But if you have a Ranked question (order the steps in the correct sequence), then the correct choice column won't matter because you just input the choice columns in order at the Google Sheet.

The question type can influence how the question is rendered and how the choices are rendered. The logic for rendering question text and choices off of the Question Type column in Google Sheet is in public/assets/formatters/format-choices.js and public/assets/formatters/format-question-text.js. What happens is based on the question type, the formatter will fill in the question HTML into the Handlebar template (whether it's text, video, sound, etc). The choices is a bit more complicated. The formatter will inject a smaller Handlebars template into the question box Handlebars template before filling in other variables (such as the choices themselves). That's why the code labels it as preinject. After the Handlebars template is rendered as HTML, then the formatter will hydrate the choices UI to be interactable. Depending on the question type, you may have multiple choices that click, or you may have a ranking question where you drag and drop the choices into the correct sequence. The view model for the choices will always be the same structure at modelizeChoices. Certain question types require a button to confirm choices (such as SATA, Ranked). That's formatted in the ui object of quiz.js.
